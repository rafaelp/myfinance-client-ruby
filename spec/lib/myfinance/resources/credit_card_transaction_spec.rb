...
describe Myfinance::Resources::CreditCardTransaction, vcr: true do
  ...
  describe "#find_all" do
    ...
    context "when supplying invalid IDs" do
      subject { credit_card_transactions.find_all(nil) }
  ...
  describe "#find" do
    context "when successful" do
      subject { credit_card_transactions.find(entity_id, cct_id) }
    ...
    context "when supplying invalid IDs" do
      subject { credit_card_transactions.find(nil, nil) }
  ...
  describe "#update" do
    ...
    context "when given invalid params" do
      let(:params) { { occurred_at: "data invalida" } }
      subject { credit_card_transactions.update(entity_id, cc_id, cct_id, params) }
  ...
  describe "#destroy" do
    ...
    let(:new_cc) { credit_card_transactions.create(entity_id, params) }

    context "when successful" do
      before :each do
        credit_card_transactions.destroy(entity_id, new_cc.id)
      end

      it "does not find deleted CreditCard" do
        expect { credit_card_transactions.find(entity_id, new_cc.id) }.to raise_error(request_error) do |e|
          expect(e.code).to eq(404)
          expect(e.message).to eq("Not Found")
        end
      end
    ...
    context "when given invalid ID" do
      it "returns 404 Not Found" do
        expect { credit_card_transactions.destroy(entity_id, 424242) }.to raise_error(request_error) do |e|
          expect(e.code).to eq(404)
          expect(e.message).to eq("Not Found")
        end
      end
